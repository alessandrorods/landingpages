...
export async function POST(
  ...
  let body: { reason?: string; notes?: string; photoUrls?: string[]; lat?: number; lng?: number }
  try {
  ...
  const { reason, notes, photoUrls = [], lat, lng } = body
  if (!reason?.trim()) {
  ...
  try {
    ...
    await orderService.markUndelivered(
      id,
      { reason: reason.trim(), notes: notes?.trim(), photoUrls, lat, lng },
    ...
    console.error(`[undelivered] id=${id} erro`, err)
