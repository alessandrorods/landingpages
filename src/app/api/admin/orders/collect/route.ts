...
export async function POST(request: NextRequest) {
  ...
  const { orderId } = body
  if (!orderId) return NextResponse.json({ error: 'Campos obrigatórios ausentes' }, { status: 400 })
  ...
  const tag = `[collect] pedido=${orderId} courier=${username}`
  ...
  try {
    ...
    if (!order) {
      console.error(tag, 'pedido não encontrado')
    ...
    await orderService.dispatch(order.id, courier.id, courierName, { type: 'user', name: courierName, role })
